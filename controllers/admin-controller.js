...
const adminChangeBanner = async (req, res) => {
  try {
    ...
    res
      .status(200)
      .json({ message: "Banner Images saved successfully.", success: true });
  } catch (error) {
...
//Update enquiry -> just normally update the is replied field
const updateEnquiry = async (req, res) => {
  try {
    ...
    res.status(200).json({ success: true, updatedEnquiry });
  } catch (error) {
    res.status(500).json({ success: false, message: error.message });
  }
};
